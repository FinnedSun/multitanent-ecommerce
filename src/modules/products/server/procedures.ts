  ...
  getOne: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const product = await ctx.db.findByID({
        collection: "products",
        id: input.id,
        depth: 2, // Load the "product.image", "product.tanant", "product.tanant.image"
      })
  ...
  getMany: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const data = await ctx.db.find({
        collection: 'products',
        depth: 2, // populate "category", "image", "tenent" && "image"
        where,
        sort,
        page: input.cursor,
        limit: input.limit,
      ...
      const dataWithSummariezedReviews = await Promise.all(
          ...
          return {
            ...
            reviewsRating:
              reviewsData.docs.length === 0
                ? 0
                : reviewsData.docs.reduce((acc, review) => acc + review.rating, 0) / reviewsData.docs.length
          }
        })
      )
      ...
      return {
        ...data,
        docs: dataWithSummariezedReviews.map((doc) => ({
          ...doc,
